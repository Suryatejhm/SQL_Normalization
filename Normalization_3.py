...
def format_sample_fields(format_field, sample_field):
    ...
    for key, value in sample_field.items():
        ...
        output[key] = dict(zip(format_field, value.split(":")))
...
def create_dict_from_line(header, line):
    ...
    for i in line:
        if "\n" in i:
            line[line.index(i)] = i.replace("\n", "")

    output = dict(zip(header, line[:8]))

    format_field = line[8]
    sample_field = dict(zip(header[9:], line[9:]))
...
def create_dictionary_of_info_field_values(data):
    ...
    temp_list = []
    for i in data:
        updated_data = i.split(";")
        for item in updated_data:
            temp_list.append(item.split("=", 1))
...
def determine_data_type_of_info_fields(data):
    ...
    for key, value in data.items():
        output[key] = determine_data_type_of_list(value)
...
def format_data(data, info_field_data_type):
    ...
    for i in data:
        output = {}
        for j in i["INFO"].split(";"):
            j = j.split("=", 1)[:10]
            if (len(j) == 2) and (j[1] != "."):
                output.update({j[0]: info_field_data_type[j[0]](j[1])})

        i.update({"INFO": output})

        for key, val in i.items():
            if key == "POS" or key == "QUAL":
                if key == "POS":
                    i[key] = int(val)
                else:
                    i[key] = float(val)
            ...
            result.append(
                {
                    ...
                    "ID": i["ID"],
                    "INFO": {
                        key: i["INFO"][key] for key in req_field if key in i["INFO"]
                    },
                    "POS": i["POS"],
                    "QUAL": i["QUAL"],
                    "REF": i["REF"],
...
def pull_basic_and_predictor_fields(filename):
    ...
    predictor_val = {
        ...
        "MetaLR_pred": {"D": 1, "T": 0},
        "MetaSVM_pred": {"D": 1, "T": 0},
        "MutationAssessor_pred": {"H": 1, "N": 0, "L": 0.25, "M": 0.5},
        "MutationTaster_pred": {"D": 1, "A": 1, "P": 0, "N": 0},
    ...
    for i in data:
        ...
        """ for key, val in i["INFO"].items():
            if key in list(predictor_val.keys()): """
        sum_predictor_value = 0
        for key, val in predictor_val.items():
            
            if key in list(i["INFO"].keys()):   
                temp[key] = i["INFO"][key]
                sum_predictor_value += predictor_val[key][i["INFO"][key]]      
            else:
                continue
        
        if temp != {}:
            temp.update({"sum_predictor_values": sum_predictor_value})
            temp.update(
                    {
                        "CHROM": i["CHROM"],
                        "POS": i["POS"],
...
def pull_basic_and_predictor_fields_gzip(filename):
    ...
    with gzip.open(filename, "rt") as file:
        for i in file:
            ...
            res = create_dict_from_line(header, line)
            info_field_list = create_dictionary_of_info_field_values([res['INFO']])
            for key,value in info_field_list.items():
                    for i in value:
                        info_field_list[key] = i
            res['INFO'] = info_field_list
            output.append(res)   
    
    #updated_filename = filename.replace(".vcf.gz", ".json")
    save_data_as_json(output, "mini_project1_gzip.json")

    output = pull_basic_and_predictor_fields("mini_project1_gzip.json")
    for i in output:
        for key, val in i.items():
            if key == "POS":
                i[key] = str(val)
...
def return_all_non_zero_sum_predictor_values():
    ...
    output = []
    for i in data:
        if i["sum_predictor_values"] > 0:
            output.append(i)
    save_data_as_json(output, "sum_predictor_values_gt_zero.json")
    return output
    # END SOLUTION
